...
export const AssessmentForm = () => {
  ...
  const downloadPDF = async (assessment: any) => {
    ...
    // Platform Recommendations - Map challenges to marketplace resources
    const generatePlatformRecommendations = () => {
      const recommendations: Array<{category: string; issue: string; solution: string; link: string}> = [];
      const baseUrl = window.location.origin;
      
      // Financial Health
      if ((assessment.financial_health_score || 0) < 60) {
        recommendations.push({
          category: "Financial Management",
          issue: "Improve financial health and planning",
          solution: "Access financial management tools and accounting services",
          link: `${baseUrl}/services/category/business-services`
        });
        recommendations.push({
          category: "Financial Skills",
          issue: "Build financial literacy",
          solution: "Enroll in financial management courses",
          link: `${baseUrl}/resources/category/learning-hub`
        });
      }
      
      // Governance & Compliance
      if ((assessment.governance_score || 0) < 60) {
        recommendations.push({
          category: "Legal & Compliance",
          issue: "Strengthen governance and compliance",
          solution: "Consult with legal and compliance service providers",
          link: `${baseUrl}/services/category/business-services`
        });
      }
      
      // Technology & Innovation
      if ((assessment.technology_innovation_score || 0) < 60) {
        recommendations.push({
          category: "Technology Development",
          issue: "Enhance technology capabilities",
          solution: "Access software development and tech services",
          link: `${baseUrl}/services/category/software-tools`
        });
        recommendations.push({
          category: "Digital Skills",
          issue: "Upskill your team in technology",
          solution: "Explore digital skills training programs",
          link: `${baseUrl}/resources/category/learning-hub`
        });
      }
      
      // Market Access & Demand
      if ((assessment.market_access_score || 0) < 60) {
        recommendations.push({
          category: "Market Expansion",
          issue: "Expand market reach and access",
          solution: "Connect with market access specialists",
          link: `${baseUrl}/access-to-market`
        });
        recommendations.push({
          category: "Marketing Support",
          issue: "Improve marketing and brand visibility",
          solution: "Engage marketing and growth services",
          link: `${baseUrl}/services/category/marketing-growth`
        });
      }
      
      // Operational Capacity
      if ((assessment.operational_capacity_score || 0) < 60) {
        recommendations.push({
          category: "Operations Improvement",
          issue: "Strengthen operational processes",
          solution: "Access operational management tools and templates",
          link: `${baseUrl}/resources/category/tools-downloads`
        });
      }
      
      // Growth Readiness & Strategy
      if ((assessment.growth_readiness_score || 0) < 60) {
        recommendations.push({
          category: "Strategic Planning",
          issue: "Develop growth strategy",
          solution: "Connect with business strategy mentors",
          link: `${baseUrl}/mentorship`
        });
        recommendations.push({
          category: "Business Skills",
          issue: "Build entrepreneurial capabilities",
          solution: "Complete entrepreneurship training programs",
          link: `${baseUrl}/resources/category/learning-hub`
        });
      }
      
      // Funding (if overall score is below 600)
      if ((assessment.overall_score || 0) < 600) {
        recommendations.push({
          category: "Funding Readiness",
          issue: "Prepare for funding opportunities",
          solution: "Explore funding options and improve your profile",
          link: `${baseUrl}/funding/browse`
        });
      }
      
      // General mentorship recommendation
      if (recommendations.length > 0) {
        recommendations.push({
          category: "Expert Guidance",
          issue: "Get personalized support",
          solution: "Book 1:1 mentorship sessions with industry experts",
          link: `${baseUrl}/find-mentor`
        });
      }
    ...
    const platformRecs = generatePlatformRecommendations();
    ...
    if (platformRecs.length > 0) {
      ...
      pdf.setFont(undefined, 'normal');
      pdf.setTextColor(...brandGrey);
      pdf.text("Based on your assessment, here are specific resources available on 22 On Sloane:", 15, yPos);
      yPos += 8;
      
      platformRecs.forEach((rec, index) => {
        ...
        pdf.setFillColor(...brandOrange);
        const titleWidth = pdf.getTextWidth(`${index + 1}. ${rec.category}`) + 4;
        pdf.roundedRect(15, yPos - 4, titleWidth, 6, 1, 1, 'F');
        
        pdf.setFont(undefined, 'bold');
        pdf.setFontSize(10);
        pdf.setTextColor(255, 255, 255);
        pdf.text(`${index + 1}. ${rec.category}`, 17, yPos);
        yPos += 6;
        ...
        pdf.setFontSize(8);
        pdf.setTextColor(...brandGrey);
        const issueLines = pdf.splitTextToSize(`Challenge: ${rec.issue}`, 165);
        ...
        // Solution
        pdf.setFont(undefined, 'normal');
        const solutionLines = pdf.splitTextToSize(`Solution: ${rec.solution}`, 165);
        ...
        pdf.setFont(undefined, 'bold');
        pdf.setFontSize(9);
        pdf.setTextColor(41, 128, 185); // Blue color for links
        pdf.textWithLink("→ Access Resource", 20, yPos, { url: rec.link });
        pdf.setTextColor(...brandGrey);
        yPos += 8;
      });
    }
